...
do {								\
	char errbuf[128];					\
								\
	printk(KERN_CRIT "%s: " _fmt ": %s" MPNL,		\
...
do {								\
	char errbuf[128];					\
								\
	printk(KERN_ERR "%s: " _fmt ": %s" MPNL,		\
	       __func__, ## __VA_ARGS__,			\
	       merr_strinfo((_err), errbuf, sizeof(errbuf)));	\
} while (0)

#define mp_pr_warn(_fmt, ...)					\
	printk(KERN_WARNING "%s: " _fmt MPNL, __func__, ## __VA_ARGS__)

#define mp_pr_notice(_fmt, ...)					\
	printk(KERN_NOTICE "%s: " _fmt MPNL, __func__, ## __VA_ARGS__)

#define mp_pr_info(_fmt, ...)					\
	printk(KERN_INFO "%s: " _fmt MPNL, __func__, ## __VA_ARGS__)

#define mp_pr_debug(_fmt, _err, ...)				\
do {								\
	char errbuf[128];					\
								\
	printk(KERN_DEBUG "%s: " _fmt ": %s" MPNL,		\
	       __func__, ## __VA_ARGS__,			\
	       merr_strinfo((_err), errbuf, sizeof(errbuf)));	\
} while (0)
